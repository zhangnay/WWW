...
class OaMySQLReback {
    ...
    private function connect() {
        if (mysql_connect($this->config['host'] . ':' . $this->config['port'], $this->config['userName'], $this->config['userPassword'])) {
            ...
            $this->throwException(L('c_success'));
        }
    }
    ...
    public function backup() {
        $this->content = '/* This file is created by OaMySQLReback ' . date('Y-m-d H:i:s') . ' */';
            ...
            if ($row = mysql_fetch_row($rs)) {
                ...
                foreach ($tables as $table) {
                    ...
                    if ($tableRow = mysql_fetch_row($tableRs)) {
                        $this->content .= "\r\n /* 创建表结构 {$table} */";
                        $this->content .= "\r\n DROP TABLE IF EXISTS {$table};/* OaMySQLReback Separation */ {$tableRow[1]};/* OaMySQLReback Separation */";
                        ...
                        if (is_array($temp)) {
                            foreach ($temp as $v) {
                                $values = implode(',', $v) . ';/* OaMySQLReback Separation */';
                                if ($values != ';/* OaMySQLReback Separation */') {
                                    $this->content .= "\r\n /* 插入数据 {$table} */";
    ...
    public function recover($fileName) {
        ...
        if (!empty($this->content)) {
            $content = explode(';/* OaMySQLReback Separation */', $this->content);
